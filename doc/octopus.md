...
现在的解决方案通过输入的爬取地址进行第一次爬取, 然后根据对应的解析结果决定是否存入url队列

之后继续进行内容爬取

现在需要继续解决下一个问题, 当内容爬取结束后, 需要进行返回上一级爬取下一页数据.


根据思路, 自己设计一个存放url, 并辅助深度搜索所使用的数据结构,或叫它一个数据存储对象 

#### 2020年6月4日15:08:59

搞了大半天发现java没有提供具体的数据结构定义, 看来只能自己封装一颗树了

#### 2020年6月5日09:51:30

事实并没有我想的那么简单. 这个数据结构遇到了很多问题. 最难搞的就是 Stack Overflow .. 原因没有细去分析
从表面上来看, 是因为在子节点引用父节点时, 放入了父节点的引用地址, 将父节点引到了子节点的对象中, 导致
父与子无限'套娃'最终栈溢出.


##### 转折出现!

突然想到放弃一个类实现, 通过将所有节点放到Map中, 然后根据引用地址作为key , 这样即可实现需要的数组+链表的自定义数据结构
此处的数据+链表与HashMap的结果相同, 但实现过程和应用并不同

改良后的octopus改的东西不是很多. 都是一些基本东西.
最终还是放弃使用上面提到的内容, 改为使用队列实现


#### 解析出来的数据处理问题

现在解析出来的内容就直接入库了. 这面想法是将它可以自定义存放, 放到数据库或放到文件中或传送给其他系统或....

所以这面引入了一个 Handler 接口
